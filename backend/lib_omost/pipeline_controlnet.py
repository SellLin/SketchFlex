...
def sample_dpmpp_2m(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    old_denoised = None
    print('the controlnet scale is: ' + str(extra_args["controlnet_conditioning_scale"]))
    print("Shape of control image:", [img.shape for img in extra_args["control_image"]])
    print('type of encoder_hiden_state:', type(extra_args["positive"]["encoder_hidden_states"]))
    for i in trange(len(sigmas) - 1, disable=disable):
        control_model_input = x
        controlnet = extra_args["controlnet_model"].to(torch.device('cuda'))
        controlnet_cond_scale = extra_args["controlnet_conditioning_scale"] 
        cond_list = [cond for _, cond in extra_args["positive"]["encoder_hidden_states"]]
        # controlnet_encoder_hiden_state = torch.cat(cond_list, dim=0)
        controlnet_encoder_hiden_state = cond_list[0]
        Transformer2DModel.forward = original_Transformer2DModel_forward

        down_block_res_samples, mid_block_res_sample = controlnet(
            control_model_input,
            sigmas[i],
            encoder_hidden_states=controlnet_encoder_hiden_state,
            controlnet_cond=extra_args["control_image"][1],
            conditioning_scale=controlnet_cond_scale,
            guess_mode=False,
            added_cond_kwargs=extra_args["positive"]["added_cond_kwargs"],
            return_dict=False,
        )
        Transformer2DModel.forward = hacked_Transformer2DModel_forward

        denoised = model(x, sigmas[i] * s_in,down_block_additional_residuals=down_block_res_samples,mid_block_additional_residual=mid_block_res_sample, **extra_args)
...
class KModel:
    ...
    def __call__(self, x, sigma, down_block_additional_residuals = None,mid_block_additional_residual= None, **extra_args):
        x_ddim_space = x / (sigma[:, None, None, None] ** 2 + self.sigma_data ** 2) ** 0.5
        t = self.timestep(sigma)
        cfg_scale = extra_args['cfg_scale']
                # Debugging prints
        print(f"ControlNet outputs:", 
              [sample.shape for sample in down_block_additional_residuals], mid_block_additional_residual.shape)
        eps_positive = self.unet(x_ddim_space, t, down_block_additional_residuals = down_block_additional_residuals,mid_block_additional_residual = mid_block_additional_residual,return_dict=False, **extra_args['positive'])[0]
...
class StableDiffusionXLOmostPipeline(StableDiffusionXLControlNetImg2ImgPipeline):
    ...
    @torch.inference_mode()
    def all_conds_from_canvas(self, canvas_outputs, negative_prompt,custom_prompts_neg,custom_masks_neg):
        # ################################################################# mask here! #################################################################
        mask_all = torch.ones(size=(1024, 1024), dtype=torch.float32)
        negative_cond, negative_pooler = self.encode_cropped_prompt_77tokens(negative_prompt)
        negative_result = [(mask_all, negative_cond)]
        for i, item in enumerate(custom_prompts_neg):
            print(i, item)
            current_mask = torch.from_numpy(custom_masks_neg[i]).to(torch.float32)
            neg_con, neg_poorler = self.encode_cropped_prompt_77tokens(item)
            negative_result.append((current_mask, neg_con))
